...
const globalFetch = typeof window !== 'undefined' && window.fetch || typeof global !== 'undefined' && global.fetch;
    ...
    if (self.queue.length > 0) {
      ...
      fetchImpl(self.endpoint, fetchOptions).then(response => {
        if (response.status !== 202 && response.status !== 200) {
          ...
          if (self.errorListenerSet) this.emit('error', new Error(err));else this.log.error(err);
        } else {
